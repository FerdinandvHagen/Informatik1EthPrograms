...
struct Shape
{
    ...
    Shape(int Type, Vector V, double D, Color aC, Color dC, Color rC) :
        type(Type), v(type == 1 ? normalize(V) : V), d(D), ambientColor(aC), diffuseColor(dC), reflectionColor(rC) {};
    ...
    {
        return type == 0 ? normalize(at-v) : v;
    }
};
...
struct Scene
{
    ...
    Scene(Vector cV, Vector lV, Color lC, int w, int h) :
        cameraVector(cV), lightVector(lV), lightColor(lC), plane_width(w), plane_height(h) {};
    ...
    {
        return Ray(cameraVector, Vector((2.0 * x + 1.0 - plane_width) / (2.0 * plane_width), (-2.0 * y -1.0 + plane_height) / (2.0 * plane_height), 0) - cameraVector);
    }
    ...
    Color trace_ray(const Ray& ray, const int depth) const
    {
        ...
        if(s != NULL)
        {
            ...
            if(illuminated)
            {
                ...
                c += lightColor*s->diffuseColor*(sR.direction*hN);
            }
            ...
            if(illuminated)
            {
                ...
                {
                    c += lightColor * s->reflectionColor * pow(rR.direction*sR.direction, 16.0);
                }
            }
